...
- 延迟探测：对目标地址做 TCP 或 ICMP 探测，可设间隔与超时，看历史曲线、丢包与可用率；一个目标可指派给多台机器，横向对比各节点；延迟高低的配色阈值可用全局默认、按目标单独设，也可存成命名方案（「优化线路」「跨洋直连」之类）给同类线路共用，改方案即改所有引用它的目标；同一目标还能按「目标 × 机器」单独覆盖阈值——美西和欧洲到同一目标可以各有一套健康标准，前台对比图在阈值不齐时自动切成按各自配色的小图
