...
import { type ProbeTargetStat, RENEW_CYCLE_LABELS, type RenewCycle } from "@/lib/types";
import { cn, fmtLatency, fmtPct } from "@/lib/utils";
...
export function ProbeTargetChip({ target }: { target: ProbeTargetStat }) {
  return (
    <div
    ...
    >
      ...
      {target.last ? (
        target.last.ok ? (
          <span className="tabular-nums">{fmtLatency(target.last.latency_ms)}</span>
        ) : (
